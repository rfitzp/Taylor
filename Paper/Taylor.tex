...
Consider an intrinsically tearing-stable  tokamak plasma that is subject to an externally generated, static (in the laboratory frame), non-axisymmetric, magnetic perturbation. The perturbation  inevitably 
...
The complex dimensionless quantities $F_s(\hat{g})$ and $\hat{\mit\Delta}_s(\hat{g})$ are determined by the layer solution in the inner region. 
...
+{\rm i}\,Q_E)^{1/4}\,[\hat{g}+{\rm i}\,(Q_E+Q_i)]^{1/4},\\[0.5ex]
F_s(\hat{g}) &= 1,\\[0.5ex]
\hat{\delta} &= \frac{|\hat{g}+{\rm i}\,Q_E|^{1/4}\,|\hat{g}+{\rm i}\,(Q_E+Q_i)|^{1/4}}{|\hat{g} +{\rm i}\,(Q_E+Q_e)|^{1/4}}.
\end{align}
Here, the resonant layer width is $\delta = r_s\,S^{-1/3}\,\hat{\delta}$. 

The {\em viscous-resistive}\/ regime is such that 
\begin{align}
\hat{\mit\Delta}_s(\hat{g}) &= \frac{6^{2/3}\,\pi\,\Gamma(5/6)}{\Gamma(1/6)}\, [\hat{g}+{\rm i}\,(Q_E+Q_e)]\,P_\varphi^{\,1/6},\\[0.5ex]
F_s(\hat{g})&= 1,\\[0.5ex]
\hat{\delta} &= P_\varphi^{\,1/6}.
\end{align}

The {\em semi-collisional}\/ regime is such that
\begin{align}
\hat{\mit\Delta}_s(\hat{g})&= \frac{\pi\,[\hat{g}+{\rm i}\,(Q_E+Q_e)]\,(\hat{g}+{\rm i}\,Q_E)^{1/2}}{D},\\[0.5ex]
F_s(\hat{g}) &= 1,\\[0.5ex]
\hat{\delta} &= \frac{|{\hat g}+{\rm i}\,Q_E|^{1/2}}{D}.
\end{align}

The {\em diffusive-resistive}\/ regime is such that
\begin{align}
\hat{\mit\Delta}_s(\hat{g})&= \frac{2\pi\,\Gamma(3/4)}{\Gamma(1/4)}\,\frac{[\hat{g}+{\rm i}\,(Q_E+Q_e)]\,\iota_e^{\,1/4}\,P_\perp^{1/4}}{D^{1/2}},\\[0.5ex]
F_s(\hat{g}) &= 1,\\[0.5ex]
\hat{\delta} &= \frac{\iota_e^{\,1/4}\,P_\perp^{1/4}}{D^{1/2}}.
\end{align}
...
\hat{\delta} &= |\hat{g}+{\rm i}\,Q_E|^{1/2}\,|\hat{g}+ {\rm i}\,(Q_E+Q_i)|^{1/2}.
\end{align}
...
\hat{\delta} &= |\hat{g}-{\rm i}\,(Q_E+Q_e)|^{1/4}\,P_\varphi^{\,1/4}.
\end{align}

Finally, the  {\em diffusive-inertial}\/ regime is such that
\begin{align}
...
\hat{\delta} &= |\hat{g}+{\rm i}\,(Q_E+Q_e)|^{1/2}\,\iota_e^{\,1/2}\,P_\perp^{\,1/2}.
\end{align}

Suppose that $|Q_E|\sim |Q_e|\sim |Q_i|\sim Q$ and $P_\varphi\sim P_\perp\sim P$. Let $R=\sqrt{\hat{g}^2+Q^2}$. The extents of the various
linear resonant response regimes in $R$-$P$ space is illustrated in Figs.~\ref{f1} and \ref{f2}. 

\section{Driven Magnetic Reconnection Regimes}
...
\end{equation}
In other words, the RMP is switched on suddenly at $t=0$. It follows from Eq.~(\ref{psibx}) that
\begin{equation}
\overline{\mit\Psi}_b(\hat{g}) = \frac{{\mit\Psi}_{b\,0}}{\hat{g}}.
\end{equation}
Let
\begin{equation}
{\mit\Psi}_0(\hat{t}) = {\mit\Psi}_\infty\,\hat{\Psi}_0(\hat{t}),
\end{equation}
where
\begin{equation}
{\mit\Psi}_\infty = \frac{E_{sb}\,{\mit\Psi}_{b\,0}}{(-E_{ss})}
\end{equation}
is the reconnected magnetic flux that would be driven at the rational surface in the absence of a current sheet at the surface. In other words, ${\mit\Psi}_\infty$ is the maximum possible
...
In the various non-constant-$\psi$ resonant response regimes (i.e., the inertial, viscous-inertial, and diffusive-inertial regimes), we shall assume that ${\mit\Sigma}\,|{\mit\Delta}_s(g)|\gg 1$, so
...
\end{equation}
In the various constant-$\psi$ regimes  (i.e., the resistive-inertial, viscous-resistive, semi-collisional, and diffusive-resistive regimes), $F_s\simeq 1$, so
\begin{equation}
...
In the inertial resonant response regime, Eqs.~(\ref{in1}), (\ref{in2}), and (\ref{ncp}) yield
\begin{equation}
\hat{\mit\Psi}_{0\,I}(\hat{t}) = \frac{1}{2\pi\,{\rm i}}\int_C \overline{\mit\Psi}_{0\,I}(\hat{g})\,{\rm e}^{\,\hat{g}\,\hat{t}}\,d\hat{g},
\end{equation}
where
\begin{align}
\overline{\mit\Psi}_{0\,I}(\hat{g}) &= \frac{\epsilon_I}{{\rm i}\,(Q_E+Q_e)}\left[
...
Now, if
\begin{equation}
\overline{\mit\Psi}_{0\,I}(\hat{g}) = \frac{\overline{H}(\hat{g})}{\hat{g} + a}
\end{equation}
then Eq.~(4.1.20) of Ref.~\onlinecite{ed} yields
\begin{equation}
\hat{\mit\Psi}_{0\,I}(\hat{t}) = \int_0^{\hat{t}}{\rm e}^{\,a\,(\hat{t}'-\hat{t})}\,H(\hat{t}')\,d\hat{t}',
\end{equation}
...
\end{equation}
However Eq.~(5.3.34) of Ref.~\onlinecite{ed} yields, 
\begin{align}
...
In the viscous-inertial resonant response regime, Eqs.~(\ref{vi1}), (\ref{vi2}), and (\ref{ncp}) yield
\begin{equation}
\hat{\mit\Psi}_{0\,VI}(\hat{t}) = \frac{1}{2\pi\,{\rm i}}\int_C \overline{\mit\Psi}_{0\,VI}(\hat{g})\,{\rm e}^{\,\hat{g}\,\hat{t}}\,d\hat{g},
\end{equation}
where
\begin{align}
\overline{\mit\Psi}_{0\,VI}(\hat{g}) =
...
\begin{equation}
{\mit\Psi}_0(t) \equiv \psi_1(0,t) = \tilde{\psi}(0,t).
\end{equation}
Let 
\begin{equation}
\overline{\mit\Psi}_0(g) = \int_0^\infty {\mit\Psi}_0(t)\,{\rm e}^{-g\,t}\,dt.
\end{equation}
It is easily demonstrated from Eqs.~(\ref{e80}) and (\ref{e314}) that\,\cite{rf1}
\begin{equation}\label{e103}
\frac{\overline{\mit\Psi}_0(g)}{\overline{\mit\Psi}_s(g)}\equiv F_s(\hat{g}) = -\int_0^\infty A(p,\hat{g})\,\frac{\partial \hat{Y}_e}{\partial p}\,dp.
\end{equation}
Thus, it follows from Eqs.~(\ref{e74}) and (\ref{e84}) 
that
\begin{equation}\label{e110}
\overline{\mit\Psi}_0(g) =\frac{ E_{sb}\,F_s(\hat{g})\,\overline{\mit\Psi}_b(g)}{S^{1/3}\,\hat{\mit\Delta}_s(\hat{g})-E_{ss}}.
\end{equation}
