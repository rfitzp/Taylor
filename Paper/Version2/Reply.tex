...
\begin{document}
\begin{center}
{\em  Forced Magnetic Reconnection due to Time-Varying Resonant Magnetic Perturbations in Flowing Tokamak Plasmas}\\[1ex]
by Y.~Lin and R.~Fitzpatrick\\[1ex]
{\bf Reply to Referees' Comments}\\[1ex]
~
\end{center}
The main criticisms of this paper is that it lacks focus on a clear physical problem, that it relies excessively on previously published research, and that
it lack novelty. We have extensively rewritten the paper to address these concerns. Our reply to these criticisms are as follows:
\begin{enumerate}

\item We have now made it very clear that the physical problem that we are addressing is the response of a flowing tokamak plasma to a resonant
magnetic perturbation that {\em varies in time}. We have changed the title of the paper to elucidate this point. We argue that, although the solution of
...
we need to add new elements to the model (for example, the new layer parameter $F_s$), and we also have to recalculate all of the layer widths in terms
...
\item[Referee 1]
\begin{enumerate}
\item We have recalculated Figs.~1-4 in the new paper using $D=2$, which is a more realistic value. The results are very similar to what we got with $D=1.2$. 
...
As is now described in the paper, the resonant layer width starts off with a comparatively large value than diminishes as time progresses. In
...
\item A constant-$\psi$ layer is a double layer, with an inner and an outer component in $p$. A non-constant-$\psi$ layer lacks the
...
These expressions are valuable because they are expresses in closed form and are sufficiently simple that they can be evaluated by a python
...
used to analyze a lot of data.

\item The key issue addresses by the paper is how to determine the response of a realistic tokamak plasma to a time-varying RMP.
...
\item The conclusions of the results is that the analytic approach gives rise to broadly similar results as the numerical approach, which is
good, because it is a lot easier to deploy the analytic method to solving the problem. 

\item The novelty of the results is that we have described a practical method for determining the response of a realistic tokamak plasma
to a time-varying RMP of arbitrary form. The practical uses of this method are fairly obvious.
